...
/**
 * Modal component shown when users attempt to access paid content
 * Displays preview status, one-time purchase option, and subscription option
 * @component BookclubPaywallModal
 * @param {Object} model.paywall - Paywall configuration data
 */
export default class BookclubPaywallModal extends Component {
  @service currentUser;
  ...
  get oneTimePrice() {
    const amount = this.args.model.paywall?.one_time_amount;
    if (!amount) {
      return null;
    }
  ...
  get subscriptionPrice() {
    const amount = this.args.model.paywall?.subscription_amount;
    if (!amount) {
      return null;
    }
    return this.formatPrice(amount);
  ...
  get subscriptionInterval() {
    return this.args.model.paywall?.subscription_interval || "month";
  }

  get hasOneTimeOption() {
    return !!this.args.model.paywall?.one_time_price_id;
  }

  get hasSubscriptionOption() {
    return !!this.args.model.paywall?.subscription_price_id;
  }
  ...
  get previewEnded() {
    const paywall = this.args.model.paywall;
  ...
  <template>
    <DModal
      @title={{I18n.t "bookclub.paywall.title"}}
      @closeModal={{@closeModal}}
      class="bookclub-paywall-modal"
    >
      <:body>
        <div class="paywall-content">
          <div class="paywall-header">
            <h2>{{@model.paywall.publication_name}}</h2>
          ...
          <div class="paywall-options">
            {{#if this.hasOneTimeOption}}
              ...
              >
                ...
                <ul class="features">
                  <li>{{I18n.t
                      "bookclub.paywall.features.permanent_access"
                    }}</li>
            ...
            {{#if this.hasSubscriptionOption}}
              ...
              >
                ...
                <ul class="features">
                  ...
                  <li>{{I18n.t
                      "bookclub.paywall.features.cancel_anytime"
                    }}</li>
                </ul>
                <DButton
