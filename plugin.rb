...
after_initialize do
  ...
  # Word count is stored on the chapter (subcategory), not the topic
  on(:post_created) do |post|
    if post.is_first_post?
      ...
      if topic.custom_fields[Bookclub::CONTENT_TOPIC] &&
           chapter&.custom_fields&.[](Bookclub::CHAPTER_ENABLED)
  ...
  add_to_class(:guardian, :can_access_publication?) do |category|
    ...
    access_tiers.each do |group_name, _level|
